...
5. Install the **e3-build-tools** tool package,
   ...
   - <https://gitlab.esss.lu.se/e3/e3-build-tools>
      ...
      ```console
      [user@host:~]$ pipx install --index-url="https://artifactory.esss.lu.se/artifactory/api/pypi/pypi-virtual/simple" e3-build-tools
      ```
...
10. Run the **e3-build** script using your preferred specification file.
   ...
   **PLC IOC**, a **SNL IOC** or a **StreamDevice IOC**.
   ...
   Below an exanple of the compilation script you may want run
   using the 2023q1_snippet.yml specification file.
   ...
   **Only for WSL1**:
   ...
   The build directory (`<epics_build_dir>`) must be
   the same you have created in point 6.

   The epics installation path (`<epics_installation_dir>`) must be
